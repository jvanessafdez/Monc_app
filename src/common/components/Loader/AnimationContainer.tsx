...
interface AnimationNode {
  ...
  values: Animated.AnimatedInterpolation<number>[]
}

export interface Props<T extends string> {
  initAnimation: () => Record<T, (value: Animated.Value) => AnimationNode>
  children: (
    interpolationsByKey: Record<T, Animated.AnimatedInterpolation<number>[]>
  ) => React.ReactNode
  animating: boolean
}
...
> extends React.Component<Props<T>> {
  ...
  > = {} as Record<T, Animated.AnimatedInterpolation<number>[]>
